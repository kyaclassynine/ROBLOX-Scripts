...
local fireSignal, rollChance do
    ...
    function rollChance()
        if (library.flags.autoPlayerMode == 'Manual') then
            ...
            if (libraryflags.goodHeld) then return 'Good' end
        ...
        for idx = 1, #arrows do
            ...
            if (arrow.Side == framework.UI.CurrentSide) and (not arrow.Marked) and framework.SongPlayer.CurrentlyPlaying.TimePosition > 0 then
                ...
                if (position) then
                    ...
                    local noteTime = math.clamp((1 - math.abs(arrow.Data.Time - (songTime + hitboxOffset))) * 99.9, 0, 99.9)
...
do
    local window = library:CreateWindow('Funky Friday') do
        local folder = window:AddFolder('Autoplayer') do
            ...
            folder:AddSlider({ text = 'Release delay (ms)', flag = 'autoDelay', min = 0, max = 250, value = 50 })
        end
